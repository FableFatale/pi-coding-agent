...
class LabelDefinition:
    ...
    count_rules: str = "once"  # once,累加,归一
    priority: int = 0  # 优先级，越高越先匹配
    exclusive: bool = False  # 是否互斥（命中后不继续匹配同类）
...
class ConditionMatcher:
    ...
    def match(self, text: str, context: Dict) -> Tuple[bool, List[str]]:
        ...
        if "min_count" in self.conditions:
            ...
            min_count = self.conditions["min_count"]
            if count >= min_count:
                matched.append(f"count>={min_count}")
        
        if "has_tag" in self.conditions:
            required_tag = self.conditions["has_tag"]
            if required_tag in context.get("tags", []):
                matched.append(required_tag)
...
class TagEngine:
    ...
    def process_turn(self, user_input: str, node: str = "", turn: int = 0):
        """处理一轮对话"""
        record = DialogRecord(turn=turn, node=node, user_input=user_input)
        ...
        for label_id, label in self.personal_labels.items():
            ...
            if matcher:
                matched, values = matcher.match(user_input, context)
                if matched:
                    # 更新计数
...
class TagParser:
    ...
    def parse_from_markdown(self, text: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        if intent_section:
            intent_labels = self._parse_label_table(intent_section, category="intent")
        
        # 解析个性标签表格
        personal_section = self._extract_section(text, "个性标签", None)
        if personal_section:
            personal_labels = self._parse_label_table(personal_section, category="personal")
    ...
    def _parse_label_table(self, section: str, category: str) -> List[Dict]:
        """解析标签表格"""
        labels = []
        lines = section.split("\n")
        ...
        for i, line in enumerate(lines):
            if "标签" in line and ("条件" in line or "语义" in line or "判定" in line):
                header_idx = i
                break
...
def create_tag_engine_from_prompt(prompt_text: str) -> Tuple[TagEngine, List[Dict], List[Dict]]:
    ...
    Returns:
        (engine, intent_labels, personal_labels)
    """
    parser = TagParser()
